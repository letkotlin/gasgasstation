 ...
 */
interface NavAdapterModel : AdapterModel<Setting>

interface NavAdapterView : AdapterView

class NavAdapter(private val items: ArrayList<Setting> = ArrayList<Setting>(),
                 private val onClick: (String, String) -> Unit) : RecyclerView.Adapter<NavAdapter.InitialSettingViewHolder>(), NavAdapterModel, NavAdapterView {
    ...
    class InitialSettingViewHolder(itemView: View, val onClick: (String, String) -> Unit) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: Setting?) {
            ...
            itemView.ll_root.setOnClickListener { onClick.invoke(PreferenceName.MAP_TYPE, item.name) }
        }
    }
